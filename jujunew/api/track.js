...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ success: true, tracked: dbSuccess, ip: ip || 'auto', city: geo.city, region: geo.region, country: geo.country, latitude: geo.latitude, longitude: geo.longitude, timezone: geo.timezone, device_info: userAgent.slice(0, 100), status: 'active' });
  } catch (error) {
    console.error('[TRACK] ❌ Unhandled:', error.message, error.stack);
    return res.status(200).json({ success: false, tracked: false, error: 'Tracking failed' });
  }
}
