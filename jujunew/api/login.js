...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2. IP extraction
    let ip = req.headers['x-real-ip'] || req.headers['x-forwarded-for'] || req.headers['client-ip'] || '';
    ...
    try {
      ...
      if (d && d.success !== false) geoData = d;
    } catch (e) { console.log('[GEO] Primary fail:', e.message); }
    ...
    if (!geoData) {
      try {
        ...
        const d = await r.json();
        if (d && !d.error) geoData = d;
      } catch (e) { console.log('[GEO] Fallback fail:', e.message); }
    }
    ...
    try {
      ...
      if (!db) { console.error('[DB] ❌ Not initialized'); }
        ...
        const { error } = await db.from('login_events').insert([{ user_id: 'arju', ip_address: ip, city, region, country, latitude, longitude, timezone, device_info, status, anomaly_status: 'normal', anomaly_reasons: '', source: 'vercel' }]);
    ...
    return res.status(isValid ? 200 : 401).json({ success: isValid, message: isValid ? 'Login successful' : 'Invalid password' });
  } catch (error) {
